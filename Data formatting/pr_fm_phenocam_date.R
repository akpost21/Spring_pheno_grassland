...
pr_fm_phenocam_date <- function(
  ...
  data_selection <- function(
    ...
    transition <- as.Date(data[,grep(sprintf("^transition_%s$",threshold),
                                     names(data))], format="%m/%d/%Y")
    lower <- as.Date(data[,grep(sprintf("*%s_lower*",threshold),names(data))], format="%m/%d/%Y")
    upper <- as.Date(data[,grep(sprintf("*%s_upper*",threshold),names(data))], format="%m/%d/%Y")
